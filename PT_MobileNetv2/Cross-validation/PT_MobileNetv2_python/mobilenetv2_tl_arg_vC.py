...
print("input_mobilenetv2")
input_mobilenet_v2 = keras.applications.mobilenet_v2.preprocess_input(np_images_resized)
...
for index in a_selected_kidneys_test:
    ...
    a_images_1D_9_kidneys = input_mobilenet_v2[bool_kidney_num]
...
#     # k fold validation  max:9 fold
    ...
    for index_val in a_kidneys_num_val:
        ...
        base_model_mobilenetv2 = keras.applications.MobileNetV2( input_shape=(224, 224, 3) ,
                                                    weights="/gpfs/alpine/bif121/proj-shared/kidney/mobilenetv2_weights_schooner/mobilenet_v2_weights_tf_dim_ordering_tf_kernels_1.0_224_no_top.h5", 
        ...
        avg = keras.layers.GlobalAveragePooling2D()(base_model_mobilenetv2.output)
        output = keras.layers.Dense(n_classes, activation="softmax")(avg)
        model_tf = keras.models.Model(inputs=base_model_mobilenetv2.input, outputs=output)
        ...
        # with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC_horovod/time_epoch_a_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
        #     np.save(f, np.array(time_cb.logs))

        with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC/history_a_K%s_outer_k%s_val'%(index, index_val), 'wb') as handle:
            pickle.dump(history.history, handle)

        with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC/pred_val_a_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
            np.save(f, y_proba)
        ...
        with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC/time_total_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
            np.save(f, np.array(time_lapse))

        # with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC_horovod/time_epoch_b_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
        #     np.save(f, np.array(time_cb.logs))

        with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC/history_b_K%s_outer_k%s_val'%(index, index_val), 'wb') as handle:
            pickle.dump(history.history, handle)

        with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/mobilenetv2_tl_results_vC/pred_val_b_K%s_outer_k%s_val.npy'%(index, index_val), 'wb') as f:
            np.save(f, y_proba)