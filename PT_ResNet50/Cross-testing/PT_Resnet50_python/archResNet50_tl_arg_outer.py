...
try:
    ...
    n_epochs_a = int(sys.argv[2])
    n_epochs_b = int(sys.argv[3])
except IndexError:
    raise SystemExit(f"Usage: {sys.argv[0]} <K_test> <n_epochs_a> <n_epochs_b>")
...
for index in a_selected_kidneys_test:
    ...
    X_train, X_test = input_resnet50[bool_kidney_train], input_resnet50[bool_kidney_test]
...
#     time_cb = TimingCallback()
    ...
    t1_stop_a = perf_counter()
    time_lapse_a = t1_stop_a-t1_start

    model_tf.save("/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/model_outer_a_K%s.h5"%(index))


    y_proba = model_tf.predict(X_test)

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/time_a_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, np.array(time_lapse_a))

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/history_a_K%s_outer'%(index), 'wb') as handle:
        pickle.dump(history.history, handle)

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/pred_val_a_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, y_proba)
    ...
    time_lapse_b = t1_stop_a-t1_start_b

    time_lapse = t1_stop-t1_start

    model_tf.save("/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/model_outer_b_K%s.h5"%(index))

    y_proba = model_tf.predict(X_test)

    print( "Elapsed time during the whole program in seconds for K%s_outer: " %(index) , time_lapse)

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/time_b_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, np.array(time_lapse_b))

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/time_total_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, np.array(time_lapse))

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/history_b_K%s_outer'%(index), 'wb') as handle:
        pickle.dump(history.history, handle)

    with open('/gpfs/alpine/bif121/proj-shared/kidney/kidney_results/archRESNET50_tf_outer_results/pred_val_b_K%s_outer.npy'%(index), 'wb') as f:
        np.save(f, y_proba)